...
(function (window) {
    ...
    document.addEventListener("DOMContentLoaded", async () => {
        ...
        // 3. Khởi tạo kết nối Node-RED
        const nodeRedReady = window.NodeRedService ? window.NodeRedService.init() : false;
        ...
        // 6. Nhận dữ liệu thực tế từ Node-RED
        if (nodeRedReady) {
            // Gắn hàm xử lý dữ liệu để file nodeRedClient.js gọi mỗi khi có tin nhắn tới
            window.onCloudSensorData = processIncomingSensorData;
        }

        // Nạp dữ liệu giả lập cho biểu đồ lịch sử ban đầu (do chưa tạo database cục bộ)
        loadMockHistory();
        
        if (window.APP_CONFIG.ENABLE_SIMULATOR && !nodeRedReady) {
